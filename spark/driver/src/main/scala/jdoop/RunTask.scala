...
abstract class RunTask(task: Task) {
  ...
  def run(): Unit = {
    ...
    // Record the number of test cases (failed and total)
    try {
      val failedTCCountFile = new File(
        Main.mkFilePath(task.hostWorkDir, "failed-test-case-count.txt")
      )
      failedTCCountFile.createNewFile()
      printToFile(failedTCCountFile){_.println(failedTestCaseCount)}
